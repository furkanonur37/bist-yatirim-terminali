...
if st.session_state['aktif_kullanici'] is None:
    ...
    with col2:
        with st.form(key="giris_formu"):
            giris_adi = st.text_input("👤 Kullanıcı Adı (Örn: ahmet123, patron)", placeholder="Kullanıcı adınızı yazın...")
    ...
    st.stop() # Kullanıcı giriş yapmadıysa uygulamanın geri kalanını yükleme!
...
# --- GELİŞMİŞ VERİ TABANI (ÇOKLU KULLANICI DESTEĞİ) ---
def tum_veritabanini_yukle():
    ...
    with open(VERI_DOSYASI, "r") as file:
        try:
            data = json.load(file)
            # Eski tek kullanıcılı sistemi yeniye entegre etme (Hata önleyici)
...
# Eğer kullanıcı ilk defa giriyorsa ona özel kasa oluştur
...
if ekle_btn and varlik_kod_input:
    ...
    portfoyler[aktif_liste][islem_kodu] = {
        "Adet": varlik_adet_input, "Maliyet": varlik_maliyet_input,
        "Hedef": hedef_fiyat, "Tip": varlik_tipi
    }
    tum_veritabanini_kaydet(tum_veritabani) 
    st.rerun() 
...
if aktif_portfoy:
    ...
    if st.sidebar.button("İşlemi Kapat ve Arşivle") and satilacak_varlik != "Seçiniz...":
        ...
        islem_kaydi = {
            ...
            "Varlık": satilacak_varlik, "Adet": adet, "Maliyet": maliyet,
            "Satış": satis_fiyati, "Kâr/Zarar": kar_zarar, "Yüzde": yuzde,
            "Durum": "Kazanıldı 🟢" if kar_zarar > 0 else "Kaybedildi 🔴"
        }
...
# --- ANA EKRAN SEKMELERİ ---
tab1, tab2, tab3, tab4, tab5 = st.tabs([f"📊 {aktif_liste} Özeti", "🤖 Algoritmik Asistan", "🏆 İşlem Geçmişi", "🏢 Canlı Piyasa", "📈 Grafik Terminali"])
...
with tab1:
    if not aktif_portfoy:
        ...
        for v in varliklar:
            try:
                ...
                veri_listesi.append({
                    ...
                    "HACİM (LOT)": f"{gunluk_hacim:,}", "TEKNİK DURUM": "🟢 Pozitif (SMA Üstü)" if guncel_fiyat > sma20 else "🔴 Negatif",
                    "DURUM": durum
                })
            except:
                pass
        ...
        if veri_listesi:
            ...
            st.markdown("---")
            st.dataframe(
                df_portfoy.style.map(soft_color_kâr, subset=['KÂR/ZARAR', 'KÂR(%)'])
                .format({
                    "MALİYET": "{:.2f} ₺", "GÜNCEL FİYAT": "{:.2f} ₺", "KÂR/ZARAR": "{:.2f} ₺",
                    "KÂR(%)": "% {:.2f}", "TOPLAM DEĞER": "{:.2f} ₺"
...
with tab3:
    ...
    if len(gecmis) > 0:
        ...
        st.info("Henüz kapatılmış (satılmış) bir işlemin yok. Sol panelden 'Varlık Sat' yaptığında geçmiş burada birikecek.")
        ...
        with st.spinner("Borsa İstanbul verileri hesaplanıyor..."):
            ...
            for hisse in bist_populer:
                try:
                    ...
                    if len(fiyatlar) >= 2:
                        fiyat_bugun = float(fiyatlar.iloc[-1])
                        fiyat_dun = float(fiyatlar.iloc[-2])
                        degisim_yuzde = ((fiyat_bugun - fiyat_dun) / fiyat_dun) * 100
                    else:
                        fiyat_bugun = float(fiyatlar.iloc[-1]) if not fiyatlar.empty else 0.0
                        degisim_yuzde = 0.0
            ...
            if piyasa_listesi:
                df_piyasa = pd.DataFrame(piyasa_listesi)
                df_piyasa = df_piyasa.sort_values(by="GÜNLÜK DEĞİŞİM (%)", ascending=False)
                
                st.dataframe(
                    df_piyasa.style.map(soft_highlight_degisim, subset=['GÜNLÜK DEĞİŞİM (%)'])
                    .format({"GÜNCEL FİYAT (₺)": "{:.2f} ₺", "GÜNLÜK DEĞİŞİM (%)": "% {:.2f}"}), 
                    width="stretch", hide_index=True
                )
...
with tab5:
    ...
    if grafik_motoru == "Yerli Motor (BİST Uyumlu)":
        with st.spinner(f"{secilen_grafik_adi} grafik verileri çekiliyor..."):
            ...
            if not mum_veri.empty:
                fig_candle = go.Figure(data=[go.Candlestick(
                    x=mum_veri.index, open=mum_veri['Open'].squeeze(), high=mum_veri['High'].squeeze(),
                    low=mum_veri['Low'].squeeze(), close=mum_veri['Close'].squeeze(), name="Fiyat",
        ...
        tv_sembol_son_hali = st.text_input("💡 TradingView Sembol Kodu:", value=tv_sembol)
        tv_widget = f"""
        <div class="tradingview-widget-container" style="height:100%;width:100%">
          <div id="tv_chart_custom" style="height: 600px;"></div>
          <script type="text/javascript" src="https://s3.tradingview.com/tv.js"></script>
          <script type="text/javascript">
          new TradingView.widget({{"autosize": true, "symbol": "{tv_sembol_son_hali}", "interval": "D", "timezone": "Europe/Istanbul", "theme": "dark", "style": "1", "locale": "tr", "enable_publishing": false, "backgroundColor": "rgba(0, 0, 0, 1)", "container_id": "tv_chart_custom"}});
          </script>
        </div>"""
